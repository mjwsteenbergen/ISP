...
export class DetailsListAdvancedExample extends React.Component<{}, IDetailsListAdvancedExampleState> {
  ...
  private _onSortColumn(key: string, isSortedDescending: boolean) {
    let sortedItems = _items.slice(0).sort((a: Array<any>, b: Array<any>) => (isSortedDescending ? a[key] < b[key] : a[key] > b[key]) ? 1 : -1);
...
//   }
  ...
  createListItems(count: number, startIndex = 0): any {
    return Array.apply(null, Array(count)).map((item, index) => {
      ...
      return {
        ...
        key: 'item-' + (index + startIndex) + ' ' + this.lorem(4),
        name: this.lorem(5),
        description: this.lorem(10 + Math.round(Math.random() * 50)),
        color: this._randWord(DATA.color),
        shape: this._randWord(DATA.shape),
        location: this._randWord(DATA.location),
        width: width,
        height: height
      };
    });
  ...
  lorem(wordCount: number): string {
    return Array.apply(null, Array(wordCount))
      .map(item => this._randWord(LOREM_IPSUM))
      .join(' ');
  }
  
  isGroupable(key: string): boolean {
    return key === 'color' ||
      key === 'shape' ||
      key === 'location';
  }
  
  _randWord(array: string[]) {
